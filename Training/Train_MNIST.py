...
env = os.environ.copy()
job_folder = os.path.dirname(os.path.abspath(__file__))
data_dir = args.input_dir
output_dir = env.get("OUTPUT_DIR", args.output_dir)
log_dir = env.get("LOG_DIR", args.output_dir)
...
def convnet_mnist(debug_output=True, epoch_size=5000, minibatch_size=64, max_epochs=10):
    ...
    for epoch in range(max_epochs):       # loop over epochs
        ...
        z.save(os.path.join(output_dir, "digit_epoch_{}.checkpoint".format(epoch)))
        

    # Load test data
    reader_test = create_reader(os.path.join(data_dir, 'Test-28x28_cntk_text.txt'), False, input_dim, num_output_classes)
...
if __name__=='__main__':
    ...
    if not os.path.exists(os.path.join(data_dir, 'Train-28x28_cntk_text.txt')) or \
       not os.path.exists(os.path.join(data_dir, 'Test-28x28_cntk_text.txt')):
        im.loadAllData(data_dir)
